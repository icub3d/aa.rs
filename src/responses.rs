...
impl Responses {
    ...
    /// memory so they can be queried. If the response queries jsonpath
    /// would produce more than one result, only the first result is
    /// returned. That is not to say an array wouldn't be returned but
    /// rather that if the query has multiple hits, the first hit is
    /// returned.
    pub fn apply(&mut self, s: &str) -> Result<String, Box<dyn std::error::Error>> {
        ...
        for query in rqs {
            let json = match self.contents.get(query.response) {
                Some(j) => j,
                None => {
                    // We don't have the contents, so let's get it, add
                    // it to our map and then return the reference.
                    let file = self.path.join(encode(&query.response));
                    println!("{:?}", file);
                    let file = (self.file_reader)(file)?;
                    let json: Value = from_reader(file)?;
                    self.contents.insert(query.response.to_string(), json);
                    self.contents.get(query.response).unwrap()
                }
            };

            let value = match jsonpath::select(&json, query.query) {
